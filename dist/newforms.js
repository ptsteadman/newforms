...
var prettyName = _require.prettyName;
...
var BoundField = Concur.extend({
  constructor: function BoundField(form, field, name) {
    if (!(this instanceof BoundField)) {
      return new BoundField(form, field, name);
    }
    this.form = form;
    this.field = field;
    this.name = name;
...
BoundField.prototype.data = function () {
  return this.field.widget.valueFromData(this.form.data, this.form.files, this.htmlName);
};
...
BoundField.prototype.value = function () {
  ...
  if (this.form.isInitialRender) {
    ...
    data = this.field.boundData(this.data(), object.get(this.form.initial, this.name, this.field.initial));
  }
...
BoundField.prototype.asWidget = function (kwargs) {
  ...
  if (autoId && typeof attrs.id == 'undefined' && typeof widget.attrs.id == 'undefined') {
  ...
  if (validation != 'manual' && validation.events) {
    for (var i = 0, l = validation.events.length; i < l; i++) {
      ...
      attrs[eventName] = this.form._handleFieldEvent.bind(this.form, { event: eventName });
    }
  }
...
BoundField.prototype.render = function (kwargs) {
  if (this.field.showHiddenInitial) {
    return React.createElement('div', null, this.asWidget(kwargs), this.asHidden({ onlyInitial: true }));
  }
  return this.asWidget(kwargs);
};
...
BoundField.prototype._isControlled = function (widget) {
  ...
  if (widget.isValueSettable) {
    ...
    controlled = this.field.controlled !== null ? this.field.controlled : this.form.controlled;
  }
  return controlled;
};
...
var MinLengthValidator = _require.MinLengthValidator;
var MaxLengthValidator = _require.MaxLengthValidator;
...
CharField.prototype.getWidgetAttrs = function (widget) {
  ...
  if (this.maxLength !== null && (widget instanceof TextInput || widget instanceof PasswordInput)) {
...
var CheckboxInput = Widget.extend({
  constructor: function CheckboxInput(kwargs) {
    if (!(this instanceof Widget)) {
      return new CheckboxInput(kwargs);
    }
...
CheckboxInput.prototype.render = function (name, value, kwargs) {
  ...
  var finalAttrs = this.buildAttrs(kwargs.attrs, { type: 'checkbox',
    name: name });
  if (value !== '' && value !== true && value !== false && value !== null && value !== undefined) {
...
var CheckboxSelectMultiple = SelectMultiple.extend({
  ...
  constructor: function constructor(kwargs) {
    if (!(this instanceof CheckboxSelectMultiple)) {
      return new CheckboxSelectMultiple(kwargs);
    }
...
var normaliseChoices = _require2.normaliseChoices;
...
ChoiceField.prototype.choices = function () {
...
ChoiceFieldRenderer.prototype.choiceInputs = function () {
  ...
  for (var i = 0, l = this.choices.length; i < l; i++) {
    inputs.push(this.choiceInputConstructor(this.name, this.value, object.extend({}, this.attrs), this.controlled, this.choices[i], i));
  }
  return inputs;
};
...
ChoiceFieldRenderer.prototype.choiceInput = function (i) {
  ...
  return this.choiceInputConstructor(this.name, this.value, object.extend({}, this.attrs), this.controlled, this.choices[i], i);
};
...
ChoiceFieldRenderer.prototype.render = function () {
  ...
  for (var i = 0, l = this.choices.length; i < l; i++) {
    ...
    if (is.Array(choiceLabel)) {
      ...
      var subRenderer = ChoiceFieldRenderer(this.name, this.value, attrsPlus, this.controlled, choiceLabel);
      ...
      var w = this.choiceInputConstructor(this.name, this.value, object.extend({}, this.attrs), this.controlled, choice, i);
...
var formatToArray = _require.formatToArray;
...
var ClearableFileInput = FileInput.extend({
  ...
  constructor: function ClearableFileInput(kwargs) {
    if (!(this instanceof ClearableFileInput)) {
      return new ClearableFileInput(kwargs);
    }
  ...
  templateWithInitial: function templateWithInitial(params) {
    return formatToArray('{initialText}: {initial} {clearTemplate}{br}{inputText}: {input}', object.extend(params, { br: React.createElement('br', null) }));
  },
  templateWithClear: function templateWithClear(params) {
    return formatToArray('{checkbox} {label}', object.extend(params, {
      label: React.createElement('label', { htmlFor: params.checkboxId }, params.label)
    }));
  },
  urlMarkupTemplate: function urlMarkupTemplate(href, name) {
...
ClearableFileInput.prototype.valueFromData = function (data, files, name) {
  ...
  if (!this.isRequired && CheckboxInput.prototype.valueFromData.call(this, data, files, this.clearCheckboxName(name))) {
    if (upload) {
...
var ComboField = Field.extend({
  constructor: function ComboField(kwargs) {
    if (!(this instanceof ComboField)) {
      return new ComboField(kwargs);
    }
...
var DateTimeInput = DateTimeBaseInput.extend({
  ...
  constructor: function DateTimeInput(kwargs) {
    if (!(this instanceof DateTimeInput)) {
      return new DateTimeInput(kwargs);
    }
    DateTimeBaseInput.call(this, kwargs);
  }
});
...
DecimalField.prototype.clean = function (value) {
  ...
  if (this.maxDigits !== null && this.decimalPlaces !== null && wholeDigits > this.maxDigits - this.decimalPlaces) {
...
ErrorList.prototype.asUl = function (kwargs) {
  ...
  return React.createElement('ul', { className: kwargs.className }, this.messages().map(function (error) {
...
var EMPTY_VALUES = _require.EMPTY_VALUES;
var ValidationError = _require.ValidationError;

var _require2 = require('./util');

var normaliseValidation = _require2.normaliseValidation;
...
var Field = Concur.extend({
  ...
  , constructor: function Field(kwargs) {
    ...
    for (var i = this.constructor.__mro__.length - 1; i >= 0; i--) {
      messages.push(object.get(this.constructor.__mro__[i].prototype, 'defaultErrorMessages', null));
    }
...
Field.prototype.runValidators = function (value) {
  ...
  for (var i = 0, l = this.validators.length; i < l; i++) {
    ...
    try {
      ...
      if (object.hasOwn(e, 'code') && object.hasOwn(this.errorMessages, e.code)) {
...
FileField.prototype.clean = function (data, initial) {
  ...
  if (data === ClearableFileInput.FILE_INPUT_CONTRADICTION) {
    throw ValidationError(this.errorMessages.contradiction, { code: 'contradiction' });
  }
...
var FilePathField = ChoiceField.extend({
  constructor: function FilePathField(path, kwargs) {
    if (!(this instanceof FilePathField)) {
      return new FilePathField(path, kwargs);
    }
    kwargs = object.extend({
...
var _require = require('isomorph/format');

var formatObj = _require.formatObj;
...
var cancellable = _require3.cancellable;
var debounce = _require3.debounce;
var info = _require3.info;
var warning = _require3.warning;
var normaliseValidation = _require3.normaliseValidation;
  ...
  constructor: function Form(kwargs) {
    ...
    if (is.Function(kwargs.onChange)) {
      if ('production' !== "development") {
        if (!warnedOnImpliedValidateAuto && kwargs.validation === 'auto') {
          info('Passing onChange to a Form or FormSet constructor also ' + "implies validation: 'auto' by default - you don't have " + 'to set it manually.');
    ...
    if ('production' !== "development") {
      ...
      if (!is.Function(kwargs.onChange) && this._needsOnChange()) {
        warning("You didn't provide an onChange callback for a " + this._formName() + ' which has controlled fields. This ' + 'will result in read-only fields.');
      }
    }
...
Form.prototype._copyInitialToData = function () {
  ...
  for (var i = 0, l = initialFieldNames.length; i < l; i++) {
    ...
    if (!this.fields[fieldName].widget.isValueSettable) {
      continue;
    }
...
Form.prototype._validateAsync = function (cb) {
  if (!is.Function(cb)) {
    throw new Error('You must provide a callback to validate() when a form has ' + 'asynchronous validation.');
  }
  if (this.isInitialRender) {
...
Form.prototype._cleanField = function (name) {
  ...
  var value = field.widget.valueFromData(this.data, this.files, this.addPrefix(name));
  var async = false;
  var error = null;
...
Form.prototype._getCustomClean = function (fieldName) {
  return this['clean' + fieldName.charAt(0).toUpperCase() + fieldName.substr(1)] || this['clean_' + fieldName];
};
...
Form.prototype._fieldCleaned = function (fieldName, err) {
  ...
  if (err) {
    if ("production" !== "development") {
      console.error('Error cleaning ' + this._formName() + '.' + fieldName + ':' + err.message);
    }
...
Form.prototype._handleFieldEvent = function (validation, e) {
  ...
  if (field instanceof FileField && 'files' in e.target) {
    ...
    this.files[htmlName] = field instanceof MultipleFileField ? Array.prototype.slice.call(files) : files[0];
  }
  if (this.isInitialRender) {
    this.isInitialRender = false;
  }
...
Form.prototype._immediateFieldValidation = function (fieldName) {
  ...
  this._lastValidatedData[fieldName] = this.fields[fieldName].widget.valueFromData(this.data, this.files, this.addPrefix(fieldName));
...
Form.prototype.addError = function (field, error) {
  ...
  if (object.hasOwn(error, 'errorObj')) {
    if (field !== null) {
      throw new Error("The 'field' argument to form.addError() must be null when " + "the 'error' argument contains errors for multiple fields.");
    }
...
Form.prototype.changedData = function (_hasChangedCheck) {
  ...
  for (var i = 0, l = fieldNames.length; i < l; i++) {
    ...
    if (!field.showHiddenInitial) {
      ...
      try {
        initialValue = hiddenWidget.valueFromData(this.data, this.files, initialPrefixedName);
      } catch (e) {
        if (!(e instanceof ValidationError)) {
          throw e;
        }
...
Form.prototype.isComplete = function () {
  ...
  for (var i = 0, l = fieldNames.length; i < l; i++) {
    var fieldName = fieldNames[i];
    if (this.fields[fieldName].required && typeof this.cleanedData[fieldName] == 'undefined') {
      return false;
    }
  }
  return true;
...
var FormRow = React.createClass({
  ...
  propTypes: {
    ...
    __all__: function __all__(props) {
      if (!props.bf && !props.content) {
        return new Error('Invalid props supplied to `FormRow`, either `bf` or `content` ' + 'must be specified.');
      }
      if (props.bf && props.content) {
        return new Error('Both `bf` and `content` props were passed to `FormRow` - `bf` ' + 'will be ignored.');
      }
    }
  },
...
var FormSet = Concur.extend({
  ...
  constructor: function FormSet(kwargs) {
    ...
    if (!is.Function(kwargs.form)) {
      throw new Error('A FormSet must be given a Form constructor to use, either via its ' + 'constructor\'s `form` option or via its prototype, passing a `form` ' + 'property to `FormSet.extend()`.');
    }
...
FormSet.prototype._validateAsync = function (cb) {
  if (!is.Function(cb)) {
    throw new Error('You must provide a callback to validate() when a formset or its form ' + 'has asynchronous validation.');
  }
  if (this.isInitialRender) {
    this.isInitialRender = false;
  }
...
FormSet.prototype._cleanFormset = function () {
  ...
  try {
    ...
    if (this.validateMax && totalFormCount - deletedFormCount > this.maxNum || !env.browser && this.managementForm().cleanedData[TOTAL_FORM_COUNT] > this.absoluteMax) {
      throw ValidationError('Please submit ' + this.maxNum + ' or fewer forms.', { code: 'tooManyForms' });
    }
    if (this.validateMin && totalFormCount - deletedFormCount < this.minNum) {
      throw ValidationError('Please submit ' + this.minNum + ' or more forms.', { code: 'tooFewForms' });
    }
...
FormSet.prototype.managementForm = function () {
  ...
  if (!env.browser && !this.isInitialRender) {
    form = new ManagementForm({ data: this.data, autoId: this.autoId,
      prefix: this.prefix });
    if (!form.isValid()) {
      throw ValidationError('ManagementForm data is missing or has been tampered with', { code: 'missing_management_form' });
    }
  } else {
    var initial = {};
...
FormSet.prototype.initialFormCount = function () {
  if (!env.browser && !this.isInitialRender) {
    ...
    return this.initial !== null && this.initial.length > 0 ? this.initial.length : 0;
  }
};
...
FormSet.prototype.deletedForms = function () {
  ...
  return this._deletedFormIndexes.map(function (i) {
    return forms[i];
  });
};
...
FormSet.prototype.orderedForms = function () {
  ...
  return this._ordering.map(function (ordering) {
    return forms[ordering[0]];
  });
};
...
FormSet.prototype.totalErrorCount = function () {
  return this.nonFormErrors().length() + this.errors().reduce(function (sum, formErrors) {
...
var GenericIPAddressField = CharField.extend({
  constructor: function GenericIPAddressField(kwargs) {
    if (!(this instanceof GenericIPAddressField)) {
      return new GenericIPAddressField(kwargs);
    }
    kwargs = object.extend({ protocol: 'both', unpackIPv4: false }, kwargs);
    this.unpackIPv4 = kwargs.unpackIPv4;
    this.defaultValidators = validators.ipAddressValidators(kwargs.protocol, kwargs.unpackIPv4).validators;
    CharField.call(this, kwargs);
  }
});
...
var validateIPv4Address = _require.validateIPv4Address;
...
var IPAddressField = CharField.extend({
  ...
  constructor: function IPAddressField(kwargs) {
    if (!(this instanceof IPAddressField)) {
...
var MaxValueValidator = _require.MaxValueValidator;
var MinValueValidator = _require.MinValueValidator;
var ValidationError = _require.ValidationError;
...
var MultiValueField = Field.extend({
  ...
  constructor: function MultiValueField(kwargs) {
    if (!(this instanceof Field)) {
      return new MultiValueField(kwargs);
    }
    kwargs = object.extend({ fields: [] }, kwargs);
    ...
    for (var i = 0, l = kwargs.fields.length; i < l; i++) {
      ...
      object.setDefault(f.errorMessages, 'incomplete', this.errorMessages.incomplete);
...
var MultiWidget = Widget.extend({
  constructor: function MultiWidget(widgets, kwargs) {
    if (!(this instanceof MultiWidget)) {
      return new MultiWidget(widgets, kwargs);
    }
    this.widgets = [];
...
MultiWidget.prototype.render = function (name, value, kwargs) {
  ...
  for (var i = 0, l = this.widgets.length; i < l; i++) {
    ...
    renderedWidgets.push(widget.render(name + '_' + i, widgetValue, { attrs: finalAttrs,
...
var MultipleChoiceField = ChoiceField.extend({
  ...
  constructor: function MultipleChoiceField(kwargs) {
    if (!(this instanceof MultipleChoiceField)) {
      return new MultipleChoiceField(kwargs);
    }
    ChoiceField.call(this, kwargs);
  }
});
...
var MultipleFileField = FileField.extend({
  ...
  constructor: function MultipleFileField(kwargs) {
    if (!(this instanceof MultipleFileField)) {
      return new MultipleFileField(kwargs);
    }
    FileField.call(this, kwargs);
  }
});
...
var MultipleHiddenInput = HiddenInput.extend({
  constructor: function MultipleHiddenInput(kwargs) {
    if (!(this instanceof MultipleHiddenInput)) {
      return new MultipleHiddenInput(kwargs);
    }
...
var NullBooleanField = BooleanField.extend({
  ...
  constructor: function NullBooleanField(kwargs) {
    if (!(this instanceof NullBooleanField)) {
      return new NullBooleanField(kwargs);
    }
...
var NullBooleanSelect = Select.extend({
  constructor: function NullBooleanSelect(kwargs) {
    if (!(this instanceof NullBooleanSelect)) {
      return new NullBooleanSelect(kwargs);
    }
    kwargs = kwargs || {};
...
var PasswordInput = TextInput.extend({
  constructor: function PasswordInput(kwargs) {
    if (!(this instanceof PasswordInput)) {
      return new PasswordInput(kwargs);
    }
...
var RegexValidator = _require.RegexValidator;
...
var RegexField = CharField.extend({
  constructor: function RegexField(regex, kwargs) {
    if (!(this instanceof RegexField)) {
      return new RegexField(regex, kwargs);
    }
    CharField.call(this, kwargs);
    if (is.String(regex)) {
...
var NON_FIELD_ERRORS = _require.NON_FIELD_ERRORS;

var _require2 = require('./util');

var autoIdChecker = _require2.autoIdChecker;
var getProps = _require2.getProps;
...
var RenderForm = React.createClass({
  ...
  propTypes: object.extend({}, formProps, {
    ...
    React.PropTypes.func, React.PropTypes.instanceOf(Form)]).isRequired,
...
var RenderFormSet = React.createClass({
  ...
  propTypes: object.extend({}, formsetProps, {
    ...
    React.PropTypes.func, React.PropTypes.instanceOf(FormSet)]),
    row: React.PropTypes.any // Component to render form rows
    ...
    , __all__: function __all__(props) {
      if (!props.form && !props.formset) {
        return new Error('Invalid props supplied to `RenderFormSet`, either `form` or ' + '`formset` must be specified.');
      }
    }
  }),
  ...
  getFormset: function getFormset() {
    return this.formset;
  },
  render: function render() {
...
var normaliseChoices = _require.normaliseChoices;
...
Select.prototype.renderOptions = function (additionalChoices) {
  ...
  for (var i = 0, l = choices.length, choice; i < l; i++) {
    ...
    if (is.Array(choice[1])) {
      ...
      for (var j = 0, m = optgroupChoices.length; j < m; j++) {
        optgroupOptions.push(this.renderOption(optgroupChoices[j][0], optgroupChoices[j][1]));
      }
      options.push(React.createElement('optgroup', { label: choice[0], key: choice[9] }, optgroupOptions));
    } else {
      options.push(this.renderOption(choice[0], choice[1]));
    }
  }
  return options;
};
...
var SelectMultiple = Select.extend({
  constructor: function SelectMultiple(kwargs) {
    if (!(this instanceof SelectMultiple)) {
      return new SelectMultiple(kwargs);
    }
    Select.call(this, kwargs);
  },
...
var SplitDateTimeField = MultiValueField.extend({
  ...
  constructor: function SplitDateTimeField(kwargs) {
    if (!(this instanceof SplitDateTimeField)) {
      return new SplitDateTimeField(kwargs);
    }
    kwargs = object.extend({
    ...
    kwargs.fields = [DateField({ inputFormats: kwargs.inputDateFormats,
      errorMessages: { invalid: errors.invalidDate } }), TimeField({ inputFormats: kwargs.inputTimeFormats,
...
var SplitDateTimeWidget = MultiWidget.extend({
  constructor: function SplitDateTimeWidget(kwargs) {
    if (!(this instanceof SplitDateTimeWidget)) {
      return new SplitDateTimeWidget(kwargs);
    }
    kwargs = object.extend({ dateFormat: null, timeFormat: null }, kwargs);
    var widgets = [DateInput({ attrs: kwargs.attrs, format: kwargs.dateFormat }), TimeInput({ attrs: kwargs.attrs, format: kwargs.timeFormat })];
...
SplitDateTimeWidget.prototype.decompress = function (value) {
  if (value) {
    return [new Date(value.getFullYear(), value.getMonth(), value.getDate()), new Date(1900, 0, 1, value.getHours(), value.getMinutes(), value.getSeconds())];
  }
  return [null, null];
};
...
var SplitHiddenDateTimeWidget = SplitDateTimeWidget.extend({
  constructor: function SplitHiddenDateTimeWidget(kwargs) {
    if (!(this instanceof SplitHiddenDateTimeWidget)) {
      return new SplitHiddenDateTimeWidget(kwargs);
    }
...
var TypedChoiceField = ChoiceField.extend({
  constructor: function TypedChoiceField(kwargs) {
    if (!(this instanceof TypedChoiceField)) {
      return new TypedChoiceField(kwargs);
    }
    kwargs = object.extend({
...
var TypedMultipleChoiceField = MultipleChoiceField.extend({
  constructor: function TypedMultipleChoiceField(kwargs) {
    if (!(this instanceof TypedMultipleChoiceField)) {
      return new TypedMultipleChoiceField(kwargs);
    }
    kwargs = object.extend({
      coerce: function coerce(val) {
        return val;
...
TypedMultipleChoiceField.prototype._coerce = function (value) {
  if (value === this.emptyValue || this.isEmptyValue(value) || is.Array(value) && !value.length) {
    return this.emptyValue;
  }
  var newValue = [];
  for (var i = 0, l = value.length; i < l; i++) {
...
var URLValidator = _require.URLValidator;
...
var URLInput = TextInput.extend({
  constructor: function URLInput(kwargs) {
    if (!(this instanceof URLInput)) {
      return new URLInput(kwargs);
    }
    TextInput.call(this, kwargs);
  },
  inputType: 'url'
